...
it('issues every due invoice and reports the count', function (): void {
    ...
    Invoice::factory()->for($reseller)->create(['period_end' => now()->subDay(), 'total_cents' => 1500]);
    Invoice::factory()->for($reseller)->create(['period_end' => now()->subDay(), 'total_cents' => 2500]);
    // Not ready: still within its period.
    Invoice::factory()->for($reseller)->create(['period_end' => now()->addDay(), 'total_cents' => 1500]);
