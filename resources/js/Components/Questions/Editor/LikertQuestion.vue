...
<template>
    <div class="mb-3">
        <label class="form-label">Scale points</label>
        <div v-for="(point, index) in scale" :key="index" class="input-group mb-2">
            ...
            />
            <button type="button" class="btn btn-outline-danger" @click="removePoint(index)">Remove</button>
        </div>
        <button type="button" class="btn btn-outline-secondary btn-sm" @click="addPoint">Add scale point</button>
    </div>
</template>
