...
- [ ] Per-reseller email templates (overridable per notification type). Human decision: subject + full Markdown body override (not subject-only or plain-text-paragraph -- the other two options considered). Split into sub-tasks since this is real scope (a new registry, table, safe-rendering pipeline, and editor UI), not one commit:
  ...
  - [ ] Safe rendering pipeline: placeholder substitution is plain string replacement against the type's fixed whitelist (never a templating engine like Blade on reseller-authored text -- that would be arbitrary PHP execution via `@php`/`@include`), and Markdown → HTML goes through CommonMark configured with raw HTML input disabled (Laravel's own `Markdown` class is configured for *developer*-authored Blade views, not untrusted input, so this needs its own explicit config, not reuse of that default)
  - [ ] Wire `UserInvited` to use the reseller's override when one exists, falling back to the existing default Blade view otherwise
