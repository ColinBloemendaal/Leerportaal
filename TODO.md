...
- [x] Optional custom CSS field, sanitized, with a hard character limit. New `custom_css` nullable text column on `reseller_themes`. Chose rejection over silent sanitization: `App\Rules\SafeCustomCss` (new `App\Rules` namespace) rejects, via a `$fail` message, values containing `</style` (breakout -- the critical one), `<script`, `@import`, `expression(`, `behavior:`, `javascript:`, `vbscript:`, or `data:text/html`, rather than quietly stripping them -- silently mangling admin-submitted CSS is worse UX and less safe than a clear rejection with a reason. 10,000-character hard limit on the field itself. `ThemeCssGenerator` appends the value after the `:root` block, re-running the same denylist itself before rendering (exposed as `SafeCustomCss::firstViolation()`, reused by both the write-side rule and this read-side check) so a row that somehow predates the rule still can't reach the page -- consistent with the generator's existing "never trust the database" stance for colors/fonts. Live preview in the theme editor injects the typed CSS into an actual `<style>` tag in the current document (not a security boundary, just a mirror of the color-field preview pattern -- the server-side rule is what actually gates what gets persisted and served to other visitors)
...
- [ ] Per-reseller email templates (overridable per notification type). Human decision: subject + full Markdown body override (not subject-only or plain-text-paragraph -- the other two options considered). Split into sub-tasks since this is real scope (a new registry, table, safe-rendering pipeline, and editor UI), not one commit:
  ...
  - [ ] Editor UI: per-type list showing default vs. overridden, subject + Markdown body fields, rendered HTML preview using sample placeholder values, reset-to-default action
  - [ ] Tests: repository, action, renderer (unsafe Markdown/HTML input is escaped, not executed), full FormRequest → DTO → Action → Repository chain, Mailable envelope/content with and without an override
